...
class TrafficLight:
    ...
    def running(self):
        for k, value in self.__color.items():
            sleep(value)
            print(k)
...
class Road:
    _spec_grav: float = 0.05
    def __init__(self, length: [int, float], width: [int, float]):
        self._length = float(length)
        self._width = float(width)

    def masf(self, tinckness: float):
        print(f'Масса асфальта состовляет: {self._length * self._width * tinckness * self._spec_grav}')
...
work = Position('Василий', 'Дружинин', 'Бухгалтер', 50000, 10000)
...
class TownCar(Car):
    ...
    def show_speed(self):
        ...
        if self.speed > 60:
            print(f'Автомобиль {self.name} движется со скоростью выше 40 км в час, сбавьте скорость')
        else:
            print(f'Автомобиль {self.name} движется с нормальной скоростью')
...
car2 = TownCar(30, 'White', 'Oka', False)
car3 = WorkCar(70, 'Rose', 'Lada', True)
car4 = PoliceCar(110, 'Blue',  'Ford', True)
print(car1.turn_left())
print(f'When {car3.turn_right()}, then {car2.stop()}')
print(f'{car4.go()} with speed exactly {car4.show_speed()}')
print(f'{car1.name} is {car1.color}')
print(f'Is {car3.name} a police car? {car3.is_police}')
print(f'Is {car2.name}  a police car? {car2.is_police}')
...
class Pencil(Stationery):
    ...
    def draw(self):
        return f'Вы используете {self.title} .Запуск отрисовки карандошом'
